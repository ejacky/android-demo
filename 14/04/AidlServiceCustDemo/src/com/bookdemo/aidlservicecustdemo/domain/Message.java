...
public class Message implements Parcelable {
	...
	public void writeToParcel(Parcel dest, int flags) {
		Log.i(TAG, "Service：Message被序列化----发件人："+fromName);
		dest.writeInt(id);
	...
	public static final Parcelable.Creator<Message> CREATOR = new Creator<Message>() {
		...
		public Message createFromParcel(Parcel source) {
			Message message=new Message(source.readInt(), source.readString(),
					source.readString(), source.readString());
			Log.i(TAG, "Service：Message被反序列化----发件人："+message.fromName);
			return message;
		}
	};
}
